...
class ProdukController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('foto')) {
            ...
            $originalFileName= date('YmdHis') . '_' . uniqid() . '_' . $extension;
    ...
    public function update(Request $request, string $id)
    {
        ...
        $messages = [
            'foto.image' => 'Format gambar gunakan file dengan ekstensi jpeg, jpg, png, atau gif.',
            'foto.max' => 'Format gambar maksimal adalah 1024 KB.'
        ];
        $validatedData['user_id'] = auth()->id();
    ...
    public function storeFoto(Request $request)
    {
        ...
        $request->validate([
            ...
            'foto_produk.*' => 'image|mimes:jpeg,jpg,pn,gif|file|max:1024',
        ]);
    ...
    {
        $kategori = Kategori::orderBy('nama_kategori', 'desc')->get();
        $produk = Produk::where('kategori_id', $id)->where('status', 1)->orderBy('update_at', 'desc')->paginate(6);
